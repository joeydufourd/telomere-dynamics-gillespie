...
numsim = 50 #number of simulations (=number of cells simulated)
...
smatrix = [
    [-1, 1, 0],  # LB->LR (APOLLO)
    [1, -1, 0],  # LR->LB (Breaks, DNAses)
    [0, -1, 1],  # LR->T (Telomere loop formation)
    [1, 0, -1],  # T->LB (T-circle, breaks)
    [0, 1, -1]   # T->LR (Opening of the t-loop, C-circle)
]

# Rate constants for each phase
k_G1  = [0.1, 0.00001, 1.0, 0.00001, 4.0]  # G1 less LB/LR more t-loops
k_S   = [1, 1, 1.0, 0.00001, 2.0] # S phase less t-loops to replicate, equilibrium for LB/LR [1.0, 1.0, 0.1, 0.00001, 2.0]
k_G2M = [0.1, 0.00001, 1.0, 0.00001, 4.0]  # G2/M back to G1 like
...
def partition_with_rare_noise(X, eps=1e-10):
    if numpy.random.random() < eps:
        return numpy.array([numpy.random.binomial(int(x), 0.5) for x in X], dtype=int)
    else:
...
def simulate_cycle(X0, tmax):
    ...
    return numpy.array(Xz), numpy.array(Yz), X
